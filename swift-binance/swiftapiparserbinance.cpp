...
void SwiftApiParserBinance::parseResponse(const quint64 &uuid, const SwiftApiClient::AsyncMethods &method, const QByteArray &data)
{
    ...
    if ( j_resp.contains("code") && j_resp.contains("msg") ) {
        ...
        if ( method == SwiftApiClient::AsyncMethods::GetOrderbooks ) {            
            ...
            QJsonArray j_symbols( j_resp.value("symbols").toArray() );
            QJsonArray items;
            QStringList unique_currencies;
            for( auto it = j_symbols.begin(); it!=j_symbols.end();it++) {
                const QJsonObject j_curr( it->toObject() );
                const QString bname = j_curr.value("baseAsset").toString();
                const QString mname = j_curr.value("quoteAsset").toString();
                if ( !unique_currencies.contains( bname ) ) {
                    unique_currencies.push_back( bname );
                }
                if ( !unique_currencies.contains( mname ) ) {
                    unique_currencies.push_back( mname );
                }
            }
            QJsonArray currs;
            for( auto it = unique_currencies.begin(); it != unique_currencies.end(); it++ ) {
                const quint32 cid = SwiftCore::getAssets( true )->getCurrencyIdByName( *it, getExchangeId() );
                QJsonObject itm;
                itm["id"] = QString::number( cid );
                itm["exchange_id"] = QString::number( getExchangeId() );
                itm["name"] = *it;
                itm["withdraw_fee"] = "0.01";
                currs.push_back( itm );
            }
            QJsonObject j_ret;
            j_ret["success"] = true;
           ...
           const quint32 market_id = assets->getMarketIdByName( market_name, getExchangeId() );
               ...
               j_ret["fee"] = QString::number( SwiftCore::getExchangeFee( getExchangeId() ), 'f', 8 );
            ...
            for( auto it = j_objects.begin(); it != j_objects.end(); it++ ) {
                ...
                if ( market_id > 0  ) {
                    ...
                    j_item["fee"] = QString::number( SwiftCore::getExchangeFee( getExchangeId() ), 'f', 8 );
