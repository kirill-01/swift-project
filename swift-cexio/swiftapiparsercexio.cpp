    ...
    if ( j_resp.contains("error") ) {
        ...
        if ( method == SwiftApiClient::AsyncMethods::GetOrderbooks ) {
            ...
            for( auto it = j_objects.begin(); it != j_objects.end(); it++ ) {
                ...
                const quint32 mcid = SwiftCore::getAssets()->getCurrencyIdByName( j_curr.value("symbol1").toString(), getExchangeId() );
                const quint32 bcid = SwiftCore::getAssets()->getCurrencyIdByName( j_curr.value("symbol2").toString(), getExchangeId() );
                if ( bcid > 0 && mcid > 0) {
                    QJsonObject itm;
            ...
            for( auto it = j_objects.begin(); it != j_objects.end(); it++ ) {
                ...
                const quint32 mcid = SwiftCore::getAssets()->getCurrencyIdByName( j_curr.value("symbol1").toString(), getExchangeId() );
                const quint32 bcid = SwiftCore::getAssets()->getCurrencyIdByName( j_curr.value("symbol2").toString(), getExchangeId() );
                if ( !_parsed_names.contains( j_curr.value("symbol1").toString() ) ) {
                    QJsonObject itm;
                    itm["exchange_id"] = QString::number( getExchangeId() );
                    ...
                    itm["deposit_address"] = "";
                    items.push_back( itm );
                    _parsed_names.push_back( j_curr.value("symbol1").toString() );
                }
                if ( !_parsed_names.contains( j_curr.value("symbol2").toString() ) ) {
                    QJsonObject itm;
                    itm["exchange_id"] = QString::number( getExchangeId() );
                    ...
                    itm["deposit_address"] = "";
                    items.push_back( itm );
                    _parsed_names.push_back( j_curr.value("symbol2").toString() );
                }
            }
            j_ret["success"] = true;
            j_ret["async_uuid"] = QString::number( uuid );
