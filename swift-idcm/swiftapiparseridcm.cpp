...
void SwiftApiParserIDCM::parseResponse(const quint64 &uuid, const SwiftApiClient::AsyncMethods &method, const QByteArray &data)
{
    if ( method == SwiftApiClient::AsyncMethods::GetOrderbooks ) {
        ...
        QJsonArray asks;
        QJsonArray bids;
        const QJsonArray j_asks( j_resp.value("data").toObject().value("asks").toArray() );
        for( auto it = j_asks.begin(); it != j_asks.end(); it++ ) {
            asks.push_back( QJsonArray({
                 QString::number(it->toObject().value("price").toDouble(), 'f', 8),
                 QString::number(it->toObject().value("amount").toDouble(), 'f', 8)
            }));
        }
        const QJsonArray j_bids( j_resp.value("data").toObject().value("bids").toArray() );
        for( auto it = j_bids.begin(); it != j_bids.end(); it++ ) {
            bids.push_back( QJsonArray({
                 QString::number(it->toObject().value("price").toDouble(), 'f', 8),
                 QString::number(it->toObject().value("amount").toDouble(), 'f', 8)
            }));
        }
