...
{
    Q_UNUSED(j_params)
    QUrl url("https://api.hitbtc.com/api/2/public/currency");
...
{
    Q_UNUSED(j_params)
    QUrl url("https://api.hitbtc.com/api/2/public/symbol");
    QNetworkRequest request( url );
    QNetworkReply * reply = getManager()->get( request );
    reply->setProperty("uuid", async_uuid );
}
...
{
    QUrl url("https://api.hitbtc.com/api/2/public/orderbook");
    QUrlQuery query;
    query.addQueryItem("symbols", SwiftCore::getAssets()->getMarketName( j_params.value("market_id").toString().toUInt() ) );
