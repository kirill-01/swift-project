...
void SwiftApiParserZB::parseResponse( const quint64& uuid, const SwiftApiClient::AsyncMethods &method, const QByteArray &data)
{
    if ( method == SwiftApiClient::AsyncMethods::TimeSyncMethod ) {
        ...
        QJsonObject j_res(  QJsonDocument::fromJson( data ).object() );
        const QJsonArray jasks( j_res.value("asks").toArray() );
        for( auto it = jasks.begin(); it != jasks.end();it++ ) {
            asks.push_back( QJsonArray({ QString::number( it->toArray().at(0).toDouble(), 'f', 8), QString::number(it->toArray().at(1).toDouble(), 'f', 8)}));
        }
        const QJsonArray jbids( j_res.value("bids").toArray() );
        for( auto it = jbids.begin(); it != jbids.end();it++ ) {
            bids.push_back( QJsonArray({ QString::number( it->toArray().at(0).toDouble(), 'f', 8), QString::number(it->toArray().at(1).toDouble(), 'f', 8)}));
        }
        ...
        QJsonObject j_res(  QJsonDocument::fromJson( data ).object() );
        QJsonArray items;
        for ( auto it = j_res.begin(); it != j_res.end(); it++ ) {
            const QString marketname( it.key() );
            const quint32 market_id = SwiftCore::getAssets()->getMarketIdByName( marketname, getExchangeId() );
            QJsonObject itm;
            itm["exchange_id"] = QString::number( getExchangeId() );
            itm["name"] = marketname;
            itm["id"] = QString::number( market_id );
            itm["base_currency_id"] = QString::number( SwiftCore::getAssets()->getCurrencyIdByName( marketname.split("_").at(0), getExchangeId() ) );
            itm["market_currency_id"] = QString::number( SwiftCore::getAssets()->getCurrencyIdByName( marketname.split("_").at(1), getExchangeId() ) );
            items.push_back( itm );
        }
        QJsonObject j_ret;
        j_ret["success"] = true;
        ...
        QJsonObject j_res(  QJsonDocument::fromJson( data ).object() );
        QStringList unique_currencies;
        const QStringList keys( j_res.keys() );
        for( auto it = keys.begin(); it != keys.end(); it++ ) {
            QStringList sepnme( it->split("_") );
            if ( !unique_currencies.contains( sepnme.at( 0) ) ) {
                unique_currencies.push_back( sepnme.at(0));
            }
            if ( !unique_currencies.contains( sepnme.at( 1) ) ) {
                unique_currencies.push_back( sepnme.at(1));
            }
        }
        QJsonArray items;
        for ( auto it = unique_currencies.begin(); it != unique_currencies.end(); it++ ) {
            const QString curname( *it );
        ...
        if(j_obj.contains("code") && ( j_obj.value("code").toInt() != 1000 && j_obj.value("code").toInt() != 3001 && j_obj.value("code").toInt() != 3005 ) ){
            ...
            case SwiftApiClient::AsyncMethods::OrderGet:{
                {
                    ...
                    parsed_result["market_id"] = QString::number(SwiftCore::getAssets()->getMarketIdByName(j_obj.value("currency").toString(), getExchangeId()));
            ...
            case SwiftApiClient::AsyncMethods::GetBalances: {
                ...
                for(auto it = data.cbegin(); it != data.cend(); it++) {
                    ...
                    auto item = it->toObject();
                    const quint32 cid = SwiftCore::getAssets()->getCurrencyIdByName(item.value("key").toString(), getExchangeId());
                    if ( cid > 0 ) {
        ...
        case SwiftApiClient::AsyncMethods::TradeHistory: {
            ...
            for(auto it = j_arr.cbegin(); it != j_arr.cend(); ++it) {
                ...
                if( orderStatus( item.value("status").toInt() ).toUInt() > 1 ) {
                    ...
                    item_order["market_id"] = QString::number( SwiftCore::getAssets()->getMarketIdByName( item.value("currency").toString(), getExchangeId() ) );
