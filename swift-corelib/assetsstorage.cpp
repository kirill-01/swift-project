...
public:
    ...
    QList<quint32> _enabled_exchanges;

    QHash<quint32,quint32> _pair_amount_precision;
    QHash<quint32,quint32> _pair_price_precision;

    QList<quint32> _enabled_currencies;
    QList<quint32> _enabled_pairs;

    QHash<quint32,QPair<quint32,quint32>> _arbitrage_pair_coins;
...
QJsonObject AssetsStorage::toJson() const {
    QJsonObject j_ret;

    QJsonArray j_exchs;
    const QStringList enames( getExchangesNames() );
    for( auto it = enames.begin(); it != enames.end(); it++ ) {
        QJsonObject j_itm;
        j_itm["id"] = QString::number( getExchangeId( *it ) );
        j_itm["name"] = *it;
        j_exchs.push_back( j_itm );
    }
    QJsonArray j_currencies;
    QList<quint32> _curs( getAllCurrencies() );
    for( auto it = _curs.begin(); it != _curs.end(); it++ ) {
        if ( isCurrencyActive( *it ) ) {
            QJsonObject j_itm;
            j_itm["id"] = QString::number( *it );
            j_itm["name"] = getCurrencyName( *it );
            j_itm["exchange_id"] = QString::number( getCurrencyExchangeId( *it ) );
            j_itm["coin_id"] = QString::number( getCurrencyCoin( *it ) );
            j_currencies.push_back( j_itm );
        }
    }
    QJsonArray j_pairs;
    QList<quint32> _pids( data->_enabled_pairs );
    for( auto it = _pids.begin(); it != _pids.end(); it++ ) {
        if ( isMarketActive( *it ) ) {
            QJsonObject j_itm;
            j_itm["id"] = QString::number( *it );
            j_itm["name"] = getMarketName( *it );
            j_itm["exchange_id"] = QString::number( getMarketExchangeId( *it ) );
            j_itm["arbitrage_pair_id"] = QString::number( getMarketArbitragePairId( *it ) );
            j_pairs.push_back( j_itm );
        }
    }
    j_ret["exchanges"] = j_exchs;
    j_ret["currencies"] = j_currencies;
    j_ret["pairs"] = j_pairs;
    return j_ret;
}

QList<quint32> AssetsStorage::getActiveMarkets() const {
    return data->_enabled_pairs;
}

quint32 AssetsStorage::getArbitragePairByCoins(const quint32 &base, const quint32 &market) const {
    const QPair<quint32,quint32> _cns({base,market});
    if ( data->_arbitrage_pair_coins.values().contains( _cns ) ) {
        return data->_arbitrage_pair_coins.key( _cns );
    } else {
        return 0;
    }
...
QString AssetsStorage::getCurrencyCoinName(const quint32 &currency_id) const {
    return data->_coinsname.value( getCurrencyCoin( currency_id) );
}
...
bool AssetsStorage::isMarketActive(const quint32 &market_id) const {
    return data->_enabled_pairs.contains( market_id );
}
...
QList<quint32> AssetsStorage::getCoins() const {
    return data->_coinsname.keys();
}
...
void AssetsStorage::loadAssets() {
    ...
    if ( q.exec("SELECT * FROM exchanges") ) {
        while( q.next() ) {
            ...
            data->_exchange_name[ eid ] = q.value("name").toString();
            ...
            if ( q.value("is_enabled").toUInt() == 1 ) {
                data->_enabled_exchanges.push_back( eid );
            }
        }
    }
    if ( q.exec("SELECT * FROM coins") ) {
        while( q.next() ) {
            const quint32 coid = q.value("id").toUInt();
            const QString coinname = q.value("name").toString();
            data->_coinsname[ coid ] = coinname;
        }
    }
    if ( q.exec("SELECT * FROM arbitrage_pairs") ) {
        while( q.next() ) {
            ...
            data->_arbitrage_pair_coins[ apid ] = QPair<quint32,quint32>({q.value("base_coin_id").toUInt(), q.value("price_coin_id").toUInt()});
    ...
    if ( q.exec("SELECT c.*, x.name as exchange_name FROM currencies c LEFT JOIN exchanges x ON c.exchange_id=x.id") ) {
         while( q.next() ) {
            ...
            if ( q.value("is_enabled").toUInt() == 1 && data->_enabled_exchanges.contains( _exchange_id ) ) {
                data->_enabled_currencies.push_back( _id );
            }
    ...
    if ( q.exec("SELECT m.*, e.name as exchange_name, e.is_enabled as exchvalid, m.is_enabled as marketvalid FROM pairs m LEFT JOIN exchanges e ON e.id = m.exchange_id") ) {
        while( q.next() ) {
            ...
            if (q.value("is_enabled").toUInt() == 1 && data->_enabled_exchanges.contains( exchange_id ) ) {
                data->_enabled_pairs.push_back( q.value("id").toUInt() );
            }
    ...
    const QString dbname( SwiftCore::getSettings()->value("mysql_db").toString().trimmed() );
