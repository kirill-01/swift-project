    ...
    if(method == SwiftApiClient::AsyncMethods::GetCurrencies){
        ...
        for(auto it = result.constBegin(); it != result.constEnd(); ++it){
            ...
            for(auto it2 = asks.constBegin(); it2 != asks.constEnd(); ++it2) {
                auto _ask_item = it2->toArray();
                QJsonArray _item;
                _item.append(QString::number(_ask_item.at(0).toString().toDouble(), 'f', 8));
                _item.append(QString::number(_ask_item.at(1).toString().toDouble(), 'f', 8));
                all_asks.push_back(_item);
            }
            for(auto it2 = bids.constBegin(); it2 != bids.constEnd(); ++it2) {
                auto _bid_item = it2->toArray();
                QJsonArray _item;
                _item.append(QString::number(_bid_item.at(0).toString().toDouble(), 'f', 8));
                _item.append(QString::number(_bid_item.at(1).toString().toDouble(), 'f', 8));
