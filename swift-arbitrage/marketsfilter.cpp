...
MarketsFilter::MarketsFilter(QObject *parent) : QObject(parent), is_debug( false ), session(nullptr)
{
    ...
    connect( this, &MarketsFilter::orderbooksSnapshot, this, &MarketsFilter::precessSnapShot, Qt::QueuedConnection );
}
...
void MarketsFilter::precessSnapShot(const QJsonObject &j_data) {

    QElapsedTimer t;
    t.start();
    QString _msg;
    _recalcs_counter++;
    QJsonArray asks( j_data.value("asks").toArray() );
    QJsonArray bids( j_data.value("bids").toArray() );
...
void MarketsFilter::recalcSizeSettings() {
    ...
    QString ret = SwiftBot::method("swift.rates", {}).toString();
