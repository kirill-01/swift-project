    ...
    if ( json_data.isArray() ) {
        if ( json_data.array().at(0).toString() == "error" ) {
            ...
            if ( SwiftApiClient::AsyncMethods::GetBalances == method ) {
                ...
                const QJsonArray j_r( j_resp.at(0).toArray() );
                QJsonArray j_items;
              ...
              //  [PAIR,[PLACEHOLDER,PLACEHOLDER,PLACEHOLDER,MIN_ORDER_SIZE,MAX_ORDER_SIZE,PLACEHOLDER,PLACEHOLDER,PLACEHOLDER,INITIAL_MARGIN,MIN_MARGIN]]
                for( auto it = j_r.begin(); it != j_r.end(); it++ ) {
                    QJsonObject j_item;
                    const QString name( it->toArray().at( 0 ).toString() );

                    const QString bcurname( parseCurrencyName( name.left(3) ) );
                ...
                QJsonArray j_bids;
                 for( auto it = j_resp.begin(); it != j_resp.end(); it++ ) {
                     ...
                     const double rate = itm.at(0).toDouble();
                     const double amount = itm.at(2).toDouble();
                     if ( amount > 0 ) {
                       // ask
                         j_asks.push_back( QJsonArray({ QString::number( rate, 'f',8 ), QString::number( amount , 'f', 8 ) }));
                     } else {
                         j_bids.push_back( QJsonArray({ QString::number( rate, 'f',8 ), QString::number( amount, 'f', 8).replace("-","") }));
                     }
                 }
                 // 8744.9,2,0.45603413
            ...
            } else if(SwiftApiClient::AsyncMethods::WithdrawHistory == method ){
